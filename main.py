...
async def on_ready():
    ...
    await client.change_presence(activity=discord.Game('Apollo Games | !apolo register'))
...
async def shop(ctx):
    ...
    embed.add_field(name="**🔴Foundation Token🔴**. WWorth 1 stack players choice from the dodo code", value="Price: 💰12,000", inline=True)
    embed.add_field(name="**♥Heart Token♥**. Worth 3 random stacks from the dodo code", value="Price: 💰20,000")
    embed.add_field(name="**💖Love Token💖**. Worth 15 random stacks from the dodo code", value="Price: 💰100,000", inline=False)
...
async def exchange(ctx, *, item : str):
    ...
    try:
        if (len(splitted_words) == 2):
            exchanged_item = splitted_words[0].lower()
            exchanged_note = splitted_words[1]
            item = item.lower()
            items_dict = {'soup kettle token':'99,000 Bells', 'foundation token':'1 stack of the players choice', 
                        'heart token':'3 random stacks', 'love token':'15 random stacks'}
